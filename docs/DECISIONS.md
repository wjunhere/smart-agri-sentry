...
## ADR-013：巡航植物停止可靠性（检测器恢复 + 已扫描植株避障抑制）
...
田间反复出现"摄像头识别到植株但车不停下"：plant_detector 明确出框、投票通过，但 mission_control 未触发植物停车。排查定位三个叠加原因：

1. **检测器被暂停后未恢复**：前端/系统在 MANUAL 下会暂停推理节点（`pause_detector`）省 BPU，但"何时恢复"依赖具体启动前端/原始服务调用，部分巡航开始时检测器仍是暂停 → 植物停车触发条件永不满足。
2. **已扫描植株重复触发避障**：停车扫描过的植株是"预期障碍"，再次经过时 `_front_obstacle_too_close()` 仍触发避障机动，与植物停车的节奏打架。
3. **推理节点崩溃静默失效**：vision 栈单进程崩溃后无人拉起，栈继续跑但无检测。

### 决策

1. **巡航开始强制恢复检测器**：`_prepare_autonomous_start()` 每次进入 AUTO 都调用 `_resume_detector_at_patrol_start()` 向 `pause_detector` 服务发 `data=false`；服务不可用（进程死）则打 WARN 显式告警（巡航仍可运行，但用户需知道不会停车）。恢复逻辑收口到状态机入口，不再依赖具体前端。
2. **已扫描植株避障抑制**：植物停车触发时把当前位置记入 `_scanned_plant_positions`；`_front_obstacle_too_close()` 在 `avoidance_scanned_radius`（默认 1.0m）内对已扫描位置直接放行。巡航开始清空列表（与里程计重置同步——坐标系变了）。
3. **视觉节点自愈**：`plant_detector_node`/`vision_pipeline_node` 加 `respawn=True, respawn_delay=2.0`。
4. **配套诊断**：`plant_detector_node` 投票通过/丢失各打一条边沿日志；`start_robot_stack.sh` 重复节点检查改进程数（容忍 DDS 幽灵图，捕获手动拉起重复）。

### 结果

- 巡航开始检测器必然在线（或显式告警），植物停车触发不再依赖前端实现。
- 已扫描植株不会再把车带进避障机动，扫描-继续节奏稳定。
- 视觉进程崩溃 2s 内自愈，栈不会带伤运行。

### 经验记录

- "功能在某前端可用、另一前端不可用"往往是状态机入口没收口——把不变式（巡航必须检测在线）放到状态迁移处而不是各调用方。
- 坐标相关的记忆（已扫描位置）必须和坐标系重置（里程计清零）同步清空，否则残留位置会错乱抑制行为。
